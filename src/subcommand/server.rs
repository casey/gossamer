...
impl Server {
      ...
      axum_server::Server::bind((self.address, self.http_port).into())
        .serve(
          Router::new()
            ...
            .route("/api/bookmark/:peer", get(Self::bookmark))
            .route("/api/bookmarks", get(Self::bookmarks))
            ...
            .route("/api/find/:hash", get(Self::find))
  ...
  async fn bookmark(
    ...
    peer: Path<DeserializeFromStr<Hash>>,
  ) -> ServerResult<Cbor<()>> {
    library.bookmarks.lock().unwrap().insert(**peer);
    Ok(Cbor(()))
  }

  async fn bookmarks(library: Extension<Arc<Library>>) -> ServerResult<Cbor<BTreeSet<Hash>>> {
    Ok(Cbor(library.bookmarks.lock().unwrap().clone()))
  }
